...
| **SEO 최적화 — robots.txt·sitemap.xml·favicon·OG 이미지(1200×630)·JSON-LD(WebSite+SearchAction+Organization)·한글 "로열트리" 키워드 노출·canonical·hreflang·webmanifest·아이콘 세트** | ✅ 2026-05-30 |

### SEO (2026-05-30)

| 산출물 | 내용 |
|---|---|
| `web/robots.txt` | 전체 허용 + 대형 JSON disallow + sitemap 지정 |
| `web/sitemap.xml` | `/` + `/legacy.html` |
| `web/favicon.svg` · `icon-192/512.png` · `apple-touch-icon.png` | 골드 crown+tree 마크 (Chrome headless 생성) |
| `web/og-image.png` | 1200×630 — "로열트리" 대형 타이포 + crest |
| `web/site.webmanifest` | PWA 매니페스트 (한글 name) |
| `index.html`/`legacy.html` head | title·description·keywords에 **로열트리** 노출, canonical→pages.dev, hreflang ko/x-default, OG·Twitter summary_large_image, JSON-LD 2종 |
| `app.js` `applyQueryParam()` | `?q=` 딥링크 검색 진입 (JSON-LD SearchAction 타깃) |
| **후속(사용자 단계)** | Google Search Console 등록 + sitemap 제출 + URL 검사로 색인 요청 (도메인 소유 인증 필요) |
